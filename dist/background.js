const L="LLMOrchestratorDB";const d="sessions";function p(){return new Promise((e,r)=>{const i=indexedDB.open(L,1);i.onerror=()=>r(i.error),i.onsuccess=()=>e(i.result),i.onupgradeneeded=o=>{const n=o.target.result;n.objectStoreNames.contains(d)||n.createObjectStore(d,{keyPath:"id"}).createIndex("timestamp","timestamp",{unique:!1})}})}async function $(e){const r=await p();return new Promise((i,o)=>{const c=r.transaction(d,"readwrite").objectStore(d).add(e);c.onsuccess=()=>i(),c.onerror=()=>o(c.error)})}async function E(e){const r=await p();return new Promise((i,o)=>{const c=r.transaction(d,"readonly").objectStore(d).get(e);c.onsuccess=()=>i(c.result),c.onerror=()=>o(c.error)})}async function y(e,r){const i=await p(),o=await E(e);if(!o)throw new Error("Session not found");return o.transcript.push(r),new Promise((n,s)=>{const l=i.transaction(d,"readwrite").objectStore(d).put(o);l.onsuccess=()=>n(),l.onerror=()=>s(l.error)})}async function k(){const e=await p();return new Promise((r,i)=>{const s=e.transaction(d,"readonly").objectStore(d).getAll();s.onsuccess=()=>{const c=s.result;c.sort((h,l)=>l.timestamp-h.timestamp),r(c)},s.onerror=()=>i(s.error)})}async function A(e){const r=await p();return new Promise((i,o)=>{const c=r.transaction(d,"readwrite").objectStore(d).delete(e);c.onsuccess=()=>i(),c.onerror=()=>o(c.error)})}const R={active:!1,sessionId:null,prompt:"",role:"CRITIC",rounds:3,currentRound:0,geminiTabId:null,chatGPTTabId:null,statusLog:[],isPaused:!1,humanFeedback:null};let t={...R},v=!0;function u(){chrome.storage.local.set({brainstormState:t})}async function S(){return new Promise(e=>{chrome.storage.local.get(["brainstormState"],r=>{r.brainstormState&&(t=r.brainstormState,t.active=!1,u()),v=!1,e()})})}S();chrome.sidePanel.setPanelBehavior({openPanelOnActionClick:!1});chrome.action.onClicked.addListener(e=>{e.windowId&&chrome.sidePanel.open({windowId:e.windowId}),chrome.tabs.query({},r=>{const i=r.some(n=>n.url&&n.url.includes("gemini.google.com")),o=r.some(n=>n.url&&(n.url.includes("chatgpt.com")||n.url.includes("openai.com")));i||chrome.tabs.create({url:"https://gemini.google.com/app",active:!1}),o||chrome.tabs.create({url:"https://chatgpt.com",active:!1})})});function a(e,r="info"){const i=`[${r==="info"?"Info":r==="error"?"Error":"System"}] ${e}`;console.log(i),t.statusLog.length>50&&t.statusLog.shift(),t.statusLog.push(i),u()}function g(e,r){return new Promise((i,o)=>{chrome.tabs.sendMessage(e,r,n=>{const s=chrome.runtime.lastError;i(s?null:n)})})}async function C(e){try{await chrome.scripting.executeScript({target:{tabId:e},files:["content.js"]})}catch{}}const w={CRITIC:{geminiInit:e=>`Topic: "${e}"
...
${t.humanFeedback}
---
Acknowledge the moderator's instructions and seamlessly incorporate them into your next response.`,t.humanFeedback=null,u());const h=i.chatGPTLoop(c,t.customChatGPTPrompt),l=await T(t.chatGPTTabId,h);if(!t.active)break;if(!l){a("ChatGPT produced no output. Aborting.","error");break}e=l,t.prompt=e,u(),t.sessionId&&await y(t.sessionId,{agent:"ChatGPT",text:l}).catch(()=>{}),await m(2e3)}}catch(o){a(`Loop crashed: ${o.message}`,"error")}finally{t.active=!1,a("Run completed or stopped.","system"),u()}}async function T(e,r){await C(e);let i=0,o=!1;for(;i<3&&!o;){i++;const s=await g(e,{action:"runPrompt",text:r});(s==null?void 0:s.status)==="done"?o=!0:await m(1e3)}if(!o)return a(`Failed to send prompt to tab ${e}`,"error"),"";a("Waiting for generation...","info"),await g(e,{action:"waitForDone"});const n=await g(e,{action:"getLastResponse"});return(n==null?void 0:n.text)||""}function m(e){return new Promise(r=>setTimeout(r,e))}
