const $="LLMOrchestratorDB";const h="sessions";function p(){return new Promise((e,r)=>{const i=indexedDB.open($,1);i.onerror=()=>r(i.error),i.onsuccess=()=>e(i.result),i.onupgradeneeded=o=>{const s=o.target.result;s.objectStoreNames.contains(h)||s.createObjectStore(h,{keyPath:"id"}).createIndex("timestamp","timestamp",{unique:!1})}})}async function L(e){const r=await p();return new Promise((i,o)=>{const c=r.transaction(h,"readwrite").objectStore(h).add(e);c.onsuccess=()=>i(),c.onerror=()=>o(c.error)})}async function E(e){const r=await p();return new Promise((i,o)=>{const c=r.transaction(h,"readonly").objectStore(h).get(e);c.onsuccess=()=>i(c.result),c.onerror=()=>o(c.error)})}async function y(e,r){const i=await p(),o=await E(e);if(!o)throw new Error("Session not found");return o.transcript.push(r),new Promise((s,n)=>{const l=i.transaction(h,"readwrite").objectStore(h).put(o);l.onsuccess=()=>s(),l.onerror=()=>n(l.error)})}async function k(){const e=await p();return new Promise((r,i)=>{const n=e.transaction(h,"readonly").objectStore(h).getAll();n.onsuccess=()=>{const c=n.result;c.sort((d,l)=>l.timestamp-d.timestamp),r(c)},n.onerror=()=>i(n.error)})}async function A(e){const r=await p();return new Promise((i,o)=>{const c=r.transaction(h,"readwrite").objectStore(h).delete(e);c.onsuccess=()=>i(),c.onerror=()=>o(c.error)})}const R={active:!1,sessionId:null,prompt:"",role:"CRITIC",rounds:3,currentRound:0,geminiTabId:null,chatGPTTabId:null,statusLog:[],isPaused:!1,humanFeedback:null};let t={...R},P=!0;function u(){chrome.storage.local.set({brainstormState:t})}async function S(){return new Promise(e=>{chrome.storage.local.get(["brainstormState"],r=>{r.brainstormState&&(t=r.brainstormState,t.active=!1,u()),P=!1,e()})})}S();chrome.sidePanel.setPanelBehavior({openPanelOnActionClick:!0});function a(e,r="info"){const i=`[${r==="info"?"Info":r==="error"?"Error":"System"}] ${e}`;console.log(i),t.statusLog.length>50&&t.statusLog.shift(),t.statusLog.push(i),u()}function g(e,r){return new Promise((i,o)=>{chrome.tabs.sendMessage(e,r,s=>{const n=chrome.runtime.lastError;i(n?null:s)})})}async function x(e){try{await chrome.scripting.executeScript({target:{tabId:e},files:["content.js"]})}catch{}}const I={CRITIC:{geminiInit:e=>`Topic: "${e}"
...
---
${e}
---`}};chrome.runtime.onMessage.addListener((e,r,i)=>{if(e.action==="getAllSessions")return k().then(o=>{i(o)}).catch(o=>{i([])}),!0;if(e.action==="deleteSession")return A(e.id).then(()=>{i({success:!0})}).catch(o=>{i({success:!1})}),!0;if(e.action==="getBrainstormState")return i(t),!1;if(e.action==="stopBrainstorm")return t.active=!1,a("Stopped by user.","system"),u(),i({success:!0}),!0;if(e.action==="startBrainstorm")return(async()=>{P&&await S();const{topic:o,rounds:s,role:n,customGeminiPrompt:c,customChatGPTPrompt:d,geminiTabId:l,chatGPTTabId:b}=e;if(!l||!b){i({success:!1,error:"Missing tab IDs."});return}const w=crypto.randomUUID();t={active:!0,sessionId:w,prompt:o,role:n||"CRITIC",customGeminiPrompt:c,customChatGPTPrompt:d,rounds:s||3,currentRound:0,geminiTabId:l,chatGPTTabId:b,statusLog:[],isPaused:!1,humanFeedback:null},a(`Starting run: ${s} rounds...`,"system"),u();try{await L({id:w,topic:o,role:t.role,timestamp:Date.now(),transcript:[{agent:"User",text:o}]})}catch(f){a(`Failed to create DB session: ${f.message}`,"error")}v().catch(f=>{a(`Loop fatal error: ${f.message}`,"error"),t.active=!1,u()}),i({success:!0})})(),!0;if(e.action==="continueBrainstorm")return(async()=>{if(!t.geminiTabId||!t.chatGPTTabId){i({success:!1,error:"Tab IDs missing. Start a new run instead."});return}if(t.active){i({success:!1,error:"Run is already active."});return}const o=e.additionalRounds||2;t.rounds+=o,t.active=!0,a(`Continuing run for ${o} more rounds...`,"system"),u(),v().catch(s=>{a(`Loop fatal error: ${s.message}`,"error"),t.active=!1,u()}),i({success:!0})})(),!0;if(e.action==="generateConclusion")return(async()=>{if(!t.geminiTabId){i({success:!1,error:"Gemini tab ID missing."});return}if(t.active){i({success:!1,error:"Run is active. Please wait for it to finish."});return}t.active=!0,a("Generating Final Conclusion via Gemini...","system"),u();try{const o=`The debate is now over. Here is the final thought from your collaborator:
---
${t.prompt}
---

Please synthesize everything that has been discussed into a single, highly polished, definitive Final Conclusion or Executive Summary.`,s=await T(t.geminiTabId,o);s?(a("Conclusion generated successfully.","system"),t.prompt=s):a("Failed to generate conclusion.","error")}catch(o){a(`Conclusion error: ${o.message}`,"error")}finally{t.active=!1,u()}i({success:!0})})(),!0;if(e.action==="pauseBrainstorm"){if(!t.active){i({success:!1,error:"Run is not active."});return}return t.isPaused=!0,a("Human Intervention: Run paused. Waiting for input...","system"),u(),i({success:!0}),!0}if(e.action==="resumeBrainstorm"){if(!t.active||!t.isPaused){i({success:!1,error:"Run is not paused."});return}return t.isPaused=!1,e.feedback?(t.humanFeedback=e.feedback,a("Human Intervention: Feedback received. Resuming...","system"),t.statusLog.push(`[System] Moderator: ${e.feedback}`),t.sessionId&&y(t.sessionId,{agent:"System",text:`[Moderator Intervention]
${e.feedback}`}).catch(()=>{})):a("Human Intervention: Run resumed without feedback.","system"),u(),i({success:!0}),!0}return!1});async function v(){let e=t.prompt;const r=I[t.role]?t.role:"CRITIC",i=I[r];a(`Loop started with role: ${r}`);try{for(;t.active&&t.currentRound<t.rounds;){for(t.currentRound++,u(),a(`Round ${t.currentRound} initiating...`);t.isPaused&&t.active;)await m(1e3);if(!t.active)break;a("Executing Gemini turn...");let o="",s=e;t.humanFeedback&&(s=`Here is the latest input from your collaborator:
---
${e}
---
...
${t.humanFeedback}
---
Acknowledge the moderator's instructions and seamlessly incorporate them into your next response.`,t.humanFeedback=null,u()),t.currentRound===1?o=i.geminiInit(s,t.customGeminiPrompt):o=i.geminiLoop(s,t.customGeminiPrompt);const n=await T(t.geminiTabId,o);if(!t.active)break;if(!n){a("Gemini produced no output. Aborting.","error");break}for(t.sessionId&&await y(t.sessionId,{agent:"Gemini",text:n}).catch(()=>{}),await m(2e3);t.isPaused&&t.active;)await m(1e3);if(!t.active)break;a("Executing ChatGPT turn...");let c=n;t.humanFeedback&&(c=`Here is the latest input from your collaborator:
---
${n}
---
...
${t.humanFeedback}
---
Acknowledge the moderator's instructions and seamlessly incorporate them into your next response.`,t.humanFeedback=null,u());const d=i.chatGPTLoop(c,t.customChatGPTPrompt),l=await T(t.chatGPTTabId,d);if(!t.active)break;if(!l){a("ChatGPT produced no output. Aborting.","error");break}e=l,t.prompt=e,u(),t.sessionId&&await y(t.sessionId,{agent:"ChatGPT",text:l}).catch(()=>{}),await m(2e3)}}catch(o){a(`Loop crashed: ${o.message}`,"error")}finally{t.active=!1,a("Run completed or stopped.","system"),u()}}async function T(e,r){await x(e);try{await chrome.tabs.update(e,{active:!0});const n=await chrome.tabs.get(e);n.windowId&&await chrome.windows.update(n.windowId,{focused:!0}),await m(200)}catch{a(`Failed to focus tab ${e}`,"error")}let i=0,o=!1;for(;i<3&&!o;){i++;const n=await g(e,{action:"runPrompt",text:r});(n==null?void 0:n.status)==="done"?o=!0:await m(1e3)}if(!o)return a(`Failed to send prompt to tab ${e}`,"error"),"";a("Waiting for generation...","info"),await g(e,{action:"waitForDone"});const s=await g(e,{action:"getLastResponse"});return(s==null?void 0:s.text)||""}function m(e){return new Promise(r=>setTimeout(r,e))}
